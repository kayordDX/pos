...
public static class AppDbSeed
{
    public static async Task SeedData(AppDbContext context, CancellationToken cancellationToken)
    {
        ...
        {
            var table = new Faker<Table>().RuleFor(o => o.Name, f => f.Name.JobArea()).RuleFor(o => o.Capacity, f => f.Random.Int(1, 14));
            var section = new Faker<Section>().RuleFor(o => o.Name, f => f.Name.FirstName()).RuleFor(o => o.Tables, _ => table.GenerateBetween(5, 7));
            var outlet = new Faker<Outlet>().RuleFor(o => o.Name, f => f.Company.CompanyName()).RuleFor(o => o.Sections, _ => section.GenerateBetween(3, 5));
            var business = new Faker<Business>().RuleFor(o => o.Name, f => f.Company.CompanyName()).RuleFor(o => o.Outlets, _ => outlet.GenerateBetween(2, 5));
        ...
        if (!context.Menu.Any() && 1 == 2)
        {
            ...
            await context.SaveChangesAsync(cancellationToken);
            await context.MenuItem.AddAsync(
                new MenuItem
                {
                    Name = "Coffee",
                    MenuSectionId = 4,
                    Price = (decimal)21.80,
                    Description = "This is more than just a cup of coffee, it's a moment of pure bliss",
                    DivisionId = 1,
                }
            );
            await context.MenuItem.AddAsync(
                new MenuItem
                {
                    ...
                    MenuSectionId = 4,
                    Price = (decimal)28.90,
                    Description = "Indulge in the Italian Dream: Our Delectable Cappuccino",
                    DivisionId = 1,
                }
            );
            await context.MenuItem.AddAsync(
                new MenuItem
                {
                    ...
                    MenuSectionId = 5,
                    Price = (decimal)18.80,
                    Description = "The world's most recognizable beverage",
                    DivisionId = 1,
                }
            );
            await context.MenuItem.AddAsync(
                new MenuItem
                {
                    Name = "Spier - Chardonnay Pinot Noir ",
                    MenuSectionId = 5,
                    Price = (decimal)18.80,
                    Description = "The world's most recognizable beverage",
                    DivisionId = 1,
            ...
            await context.OptionGroup.AddAsync(
                new OptionGroup
                {
                    Name = "Pizza Extras",
                    MinSelections = 0,
            ...
            await context.OrderItemStatus.AddAsync(
                new OrderItemStatus
                {
                    OrderItemStatusId = 1,
                    IsBillable = false,
            ...
            await context.SaveChangesAsync(cancellationToken);
            await context.TableBooking.AddAsync(
                new TableBooking
                {
                    TableId = 1,
                    SalesPeriodId = 1,
                    BookingName = "Seeded Booking",
                    UserId = "103301258912011927884",
                }
            );
            await context.TableBooking.AddAsync(
                new TableBooking
            ...
            await context.SaveChangesAsync(cancellationToken);
            await context.OrderItem.AddAsync(
                new OrderItem()
                {
                    MenuItemId = 1,
                    UnitPrice = 21.80M,

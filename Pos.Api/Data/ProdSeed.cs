...
public static class ProdSeed
{
    ...
    {
        // Seed Totals
        var tableBookings = await context
            .TableBooking.Where(x => x.CloseDate != null)
        ...
        {
            await context.PaymentType.AddAsync(
                new PaymentType
                {
                    PaymentTypeId = 1,
                    PaymentTypeName = "Halo",
        ...
        {
            await context.CashUpConfig.AddAsync(
                new CashUpConfig()
                {
                    Id = 1,
                    Value = -20,
                    OutletId = 1,
                    Name = "Breakage Config",
                }
            );
            await context.SaveChangesAsync(cancellationToken);
        }
        ...
        {
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ItemType = "Sales Revenue",
                    Id = 1,
                    IsAuto = true,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.SalesRevenue,
                    Position = 1,
                }
            );
            ...
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ...
                    IsAuto = true,
                    PaymentTypeId = 1,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.PaymentTotal,
                    Position = 2,
                }
            );
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ...
                    IsAuto = true,
                    PaymentTypeId = 1,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.PaymentTip,
                    Position = 3,
                }
            );
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ...
                    IsAuto = true,
                    PaymentTypeId = 1,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.PaymentLevy,
                    Position = 4,
                    AffectsGrossBalance = true,
                }
            );
            ...
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ItemType = "Breakage Fee",
                    Id = 10,
                    IsAuto = true,
                    CashupConfigId = 1,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.Config,
                    Position = 10,
                    AffectsGrossBalance = true,
                }
            );
            ...
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ItemType = "Adjustment Staff Discount",
                    Id = 12,
                    IsAuto = true,
                    AdjustmentTypeId = 2,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.Adjustment,
            ...
            await context.CashUpUserItemType.AddAsync(
                new CashUpUserItemType()
                {
                    ...
                    Id = 15,
                    IsAuto = false,
                    CashUpUserItemRule = Common.Enums.CashUpUserItemRule.None,
                    Position = 15,
                }
            );
            await context.CashUpUserItemType.AddAsync(

...
public class GetMenuItemsEndpoint : Endpoint<Request, MenuItemDTO>
{
    ...
    {
        var menuItems = await _dbContext
            .Database.SqlQuery<MenuItemResponse>(
                $"""
                    select
                        mi.menu_item_id,
                        mi.menu_section_id,
        ...
        var options = await _dbContext
            .Database.SqlQuery<OptionsResponse>(
                $"""
                    select
                        miop.menu_item_id,
        ...
        var extras = await _dbContext
            .Database.SqlQuery<ExtrasResponse>(
                $"""
                    select
                        eop.menu_item_id,
                        eop.extra_group_id,
        ...
        var optionsGrouped = options
            .GroupBy(x => x.OptionGroupId)
            .Select(
                (g, m) =>
                    new MenuItemOptionGroupDTO
                    {
                        OptionGroupId = g.Key,
                        ...
                        OptionGroup = new OptionGroupDTO
                        {
                            ...
                            Options = g.GroupBy(oo => oo.OptionId)
                                .Select(
                                    (o, om) =>
                                        new OptionDTO
                                        {
        ...
        var extrasGrouped = extras
            .GroupBy(x => x.ExtraGroupId)
            .Select(
                (g, m) =>
                    new MenuItemExtraGroupDTO
                    {
                        ExtraGroupId = g.Key,
                        MenuItemId = g.First().MenuItemId,
                        ExtraGroup = new ExtraGroupDTO
                        {
                            ...
                            Extras = g.GroupBy(oo => oo.ExtraId)
                                .Select(
                                    (o, om) =>
                                        new ExtraDTO
                                        {
                                            ...
                                            ExtraGroup = new ExtraGroupBasicDTO { ExtraGroupId = g.Key, Name = g.First().ExtraGroupName },
                                        }
                                )
                                .ToList(),
                        },
            ...
            .Select(
                (g, m) =>
                    new MenuItemDTO
                    {
                        ...
                        Tags = g.Where(t => t.TagId != null).Select(t => new Tag { Name = t.TagName ?? string.Empty, TagId = t.TagId ?? 0 }).ToList(),

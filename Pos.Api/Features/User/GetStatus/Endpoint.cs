...
public class Endpoint(AppDbContext dbContext, CurrentUserService cu) : EndpointWithoutRequest<Response>
{
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        var userRoles = await _dbContext
            .UserRoleOutlet.Where(ur => ur.UserId == _cu.UserId && ur.OutletId == userOutlet.OutletId)
        ...
        var divisions = await _dbContext
            .Database.SqlQuery<DivisionDTO>(
                $"""
                    select
        ...
        var features = await _dbContext.OutletFeature.Where(x => x.OutletId == userOutlet.OutletId).Select(x => x.Feature).ToListAsync(ct);
        resp.Features = features;

        var salesPeriod = await _dbContext
            .SalesPeriod.Where(x => x.OutletId == userOutlet.OutletId && x.StartDate != null && x.EndDate == null)
            .Select(x => new SalesPeriodDTO()
            {
                Id = x.Id,
                Name = x.Name,
                EndDate = x.EndDate,
        ...
        var clockInStatus = await _dbContext.Clock.AnyAsync(x => x.UserId == _cu.UserId && x.OutletId == userOutlet.OutletId && x.EndDate == null, ct);

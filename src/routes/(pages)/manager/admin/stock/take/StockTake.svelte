<script lang="ts">
	import { createStockItemsUpdate, type StockGetAllDivisionResponse } from "$lib/api";
	...
	let { refetch, open = $bindable(false), stockItem }: Props = $props();
...
<Dialog.Root bind:open>
	<Dialog.Content class="max-h-[98%] overflow-auto">
		<form method="POST" use:enhance>
			<Dialog.Header>
				<Dialog.Title>Edit Stock Item</Dialog.Title>
				<Dialog.Description>
					{stockItem.name}
				</Dialog.Description>
			</Dialog.Header>

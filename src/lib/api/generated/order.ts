...
export const useTableOrderCopyItemHook = () => {
	const tableOrderCopyItem = useCustomClient<TableOrderCopyItemResponse>();

	return (tableOrderCopyItemRequest: BodyType<TableOrderCopyItemRequest>) => {
		return tableOrderCopyItem({
			url: `/order/copyItem`,
			method: "POST",
			headers: { "Content-Type": "application/json" },
...
export const useTableOrderCopyItemMutationOptions = <
	TError = ErrorType<void | InternalErrorResponse>,
	TContext = unknown,
>(options?: {
	mutation?: CreateMutationOptions<
		Awaited<ReturnType<ReturnType<typeof useTableOrderCopyItemHook>>>,
		TError,
...
> => {
	...
	const tableOrderCopyItem = useTableOrderCopyItemHook();
...
>(options?: {
	...
	TContext
> => {
	const mutationOptions = useTableOrderCopyItemMutationOptions(options);

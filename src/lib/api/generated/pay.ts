...
export const usePayStatusSSEHook = () => {
	const payStatusSSE = useCustomClient<unknown>();

	return (reference: string | null) => {
		return payStatusSSE({ url: `/pay/status/sse/${reference}`, method: "GET" });
	};
};

export const getPayStatusSSEQueryKey = (reference: string | null) => {
	return [`/pay/status/sse/${reference}`] as const;
};

export const usePayStatusSSEQueryOptions = <
	TData = Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>,
	TError = ErrorType<InternalErrorResponse>,
>(
	reference: string | null,
	options?: {
		query?: Partial<
			CreateQueryOptions<Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>, TError, TData>
		>;
	}
) => {
	const { query: queryOptions } = options ?? {};

	const queryKey = queryOptions?.queryKey ?? getPayStatusSSEQueryKey(reference);

	const payStatusSSE = usePayStatusSSEHook();

	const queryFn: QueryFunction<Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>> = () =>
		payStatusSSE(reference);

	return { queryKey, queryFn, enabled: !!reference, ...queryOptions } as CreateQueryOptions<
		Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>,
		TError,
		TData
	> & { queryKey: QueryKey };
};

export type PayStatusSSEQueryResult = NonNullable<
	Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>
>;
export type PayStatusSSEQueryError = ErrorType<InternalErrorResponse>;

export const createPayStatusSSE = <
	TData = Awaited<ReturnType<ReturnType<typeof usePayStatusSSEHook>>>,
	TError = ErrorType<InternalErrorResponse>,
>(
	reference: string | null,
	...
	const queryOptions = usePayStatusSSEQueryOptions(reference, options);

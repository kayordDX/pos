<script lang="ts">
	...
	const triggerContent = $derived(stringToFDate(salesPeriods.find((f) => f.id === salesPeriod)?.startDate) ?? "Select Sales Period");
...
<Select.Root type="single" name="salesPeriod" bind:value={() => salesPeriod.toString(), (v) => (salesPeriod = Number(v))}>
	...
	<Select.Content class="max-h-100">
		<Select.Group>
			...
			{#each salesPeriods as salesPeriod (salesPeriod.id)}
				<Select.Item value={salesPeriod.id.toString()} label={salesPeriod.startDate ?? salesPeriod.id.toString()}>
